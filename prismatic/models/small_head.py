...
class Small_head_img(nn.Module):
    def __init__(
        self,
        ...
        """
        super(Small_head_img, self).__init__()
        ...
        #print(self.num_obs_features, self.obs_encoding_size) #1280 1024
        ...
        #    nn.Linear(8*self.obs_encoding_size+1, 8 * 4),          
        #)
        #
        ...
        self.action_predictor = nn.Sequential(
            nn.Linear((8+1)*self.obs_encoding_size+1, 256),   
            nn.ReLU(),            
            nn.Linear(256, 128),  
            nn.ReLU(),            
            nn.Linear(128, 64),  
        ...
        """
        self.action_predictor = nn.Sequential(
            nn.Linear(8*self.obs_encoding_size+1, self.obs_encoding_size),   
            nn.ReLU(),               
    ...
    def forward(self, obs_img: torch.tensor, vla_feature: torch.tensor, taskid: torch.tensor) -> torch.Tensor:
        ...
        tokens = torch.cat((vla_feature, obs_encoding.unsqueeze(1)), dim=1)
...
class Small_head_img_pd2_noid(nn.Module):
    def __init__(
        self,
        obs_encoding_size: Optional[int] = 512,
        mha_num_attention_heads: Optional[int] = 2,
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head_img_pd2_noid, self).__init__()
        self.obs_encoding_size = obs_encoding_size
    ...
    def forward(self, obs_img: torch.tensor, past_img: torch.tensor, vla_feature: torch.tensor) -> torch.Tensor:
        ...
        #x = torch.cat((tokens.reshape(tokens.shape[0]), dim=1)
...
class Small_head_img_pd2(nn.Module):
    def __init__(
        self,
        obs_encoding_size: Optional[int] = 512,
        mha_num_attention_heads: Optional[int] = 2,
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head_img_pd2, self).__init__()
        self.obs_encoding_size = obs_encoding_size
...
class Small_head_img_pd(nn.Module):
    def __init__(
        self,
        obs_encoding_size: Optional[int] = 512,
        mha_num_attention_heads: Optional[int] = 2,
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head_img_pd, self).__init__()
        self.obs_encoding_size = obs_encoding_size
        ...
        self.action_predictor = nn.Sequential(
            nn.Linear((8+1+1)*self.obs_encoding_size+1, 256),   
            nn.ReLU(),            
            nn.Linear(256, 128),  
            nn.ReLU(),            
            nn.Linear(128, 64),  
...
class Small_head_img_p(nn.Module):
    def __init__(
        self,
        obs_encoding_size: Optional[int] = 512,
        mha_num_attention_heads: Optional[int] = 2,
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head_img_p, self).__init__()
        self.obs_encoding_size = obs_encoding_size

        # For small head model
        self.obs_encoder = EfficientNet.from_name("efficientnet-b0", in_channels=6) # context
        self.num_obs_features = self.obs_encoder._fc.in_features
    ...
    def forward(self, obs_img: torch.tensor, past_img: torch.tensor, vla_feature: torch.tensor, taskid: torch.tensor) -> torch.Tensor:
        ...
        batch_size = obs_img.shape[0]
        cat_img = torch.cat((obs_img, past_img), dim=1)
        obs_encoding = self.obs_encoder.extract_features(cat_img)
        obs_encoding = self.obs_encoder._avg_pooling(obs_encoding)
        if self.obs_encoder._global_params.include_top:
            obs_encoding = obs_encoding.flatten(start_dim=1)
            obs_encoding = self.obs_encoder._dropout(obs_encoding)
...
class Small_head_one(nn.Module):
    def __init__(
        self,
        obs_encoding_size: Optional[int] = 512,
        mha_num_attention_heads: Optional[int] = 2,
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head_one, self).__init__()
        self.obs_encoding_size = obs_encoding_size
        ...
        self.action_predictor = nn.Sequential(
            nn.Linear(8*self.obs_encoding_size+1, 256),   
            nn.ReLU(),            
            nn.Linear(256, 128),  
            nn.ReLU(),            
            nn.Linear(128, 64),  
    ...
    def forward(self, obs_img: torch.tensor, vla_feature: torch.tensor, taskid: torch.tensor) -> torch.Tensor:
        ...
        vla_feature = self.decoder(vla_feature) 
        
        x = torch.cat((vla_feature.reshape(vla_feature.shape[0], -1), taskid.unsqueeze(1)), dim=1)
        action_pred = self.action_predictor(x).reshape(batch_size, NUM_ACTIONS_CHUNK, -1)
                   
        return action_pred


class Small_head_idcat(nn.Module):
    def __init__(
        self,
        obs_encoding_size: Optional[int] = 512,
        mha_num_attention_heads: Optional[int] = 2,
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head_idcat, self).__init__()
        self.obs_encoding_size = obs_encoding_size
        ...
        self.decoder = MultiLayerDecoder_idcat(
            embed_dim=self.obs_encoding_size,
            seq_len=8+1,
            output_layers=[256, 128, 64, 32],
            nhead=mha_num_attention_heads,
    ...
    def forward(self, obs_img: torch.tensor, vla_feature: torch.tensor, taskid: torch.tensor) -> torch.Tensor:
        ...
        #print(vla_feature.size(), obs_encoding.size())
        tokens = torch.cat((vla_feature, obs_encoding.unsqueeze(1)), dim=1)
        final_repr = self.decoder(tokens, taskid)              
        action_pred = self.action_predictor(final_repr).reshape(batch_size, NUM_ACTIONS_CHUNK, -1)
                    
        return action_pred

class Small_head_notrans(nn.Module):
    def __init__(
        self,
        obs_encoding_size: Optional[int] = 512,
        mha_num_attention_heads: Optional[int] = 2,
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head_notrans, self).__init__()
        self.obs_encoding_size = obs_encoding_size
        ...
        self.decoder = MultiLayerDecoder_notrans(
            embed_dim=self.obs_encoding_size,
            seq_len=8,
            output_layers=[256, 128, 64, 32],
            nhead=mha_num_attention_heads,
...
class Small_head(nn.Module):
    def __init__(
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head, self).__init__()
        self.obs_encoding_size = obs_encoding_size
    ...
    def forward(self, obs_img: torch.tensor, vla_feature: torch.tensor) -> torch.Tensor:
...
class Small_head_past(nn.Module):
    def __init__(
        self,
        obs_encoding_size: Optional[int] = 512,
        mha_num_attention_heads: Optional[int] = 2,
        ...
        Args:
            ...
            goal_encoding_size (int): size of the encoding of the goal images
        """
        super(Small_head_past, self).__init__()
        self.obs_encoding_size = obs_encoding_size
    ...
    def forward(self, obs_img: torch.tensor, pas_img: torch.tensor, vla_feature: torch.tensor) -> torch.Tensor:
        ...
        pas_encoding = self.obs_encoder.extract_features(pas_img)
        pas_encoding = self.obs_encoder._avg_pooling(pas_encoding)        
        if self.obs_encoder._global_params.include_top:
            obs_encoding = obs_encoding.flatten(start_dim=1)
            obs_encoding = self.obs_encoder._dropout(obs_encoding)
        if self.obs_encoder._global_params.include_top:
            pas_encoding = pas_encoding.flatten(start_dim=1)
            pas_encoding = self.obs_encoder._dropout(pas_encoding)            
            
        obs_encoding = self.compress_obs_enc(obs_encoding)            
        pas_encoding = self.compress_obs_enc(pas_encoding) 
        
        #print(vla_feature.size(), obs_encoding.size())
        tokens = torch.cat((vla_feature, obs_encoding.unsqueeze(1), pas_encoding.unsqueeze(1)), dim=1)
        final_repr = self.decoder(tokens)        
        action_pred = self.action_predictor(final_repr).reshape(batch_size, NUM_ACTIONS_CHUNK, -1)
