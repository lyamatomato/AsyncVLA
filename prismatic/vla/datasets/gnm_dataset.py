...
class GNM_Dataset_rand(Dataset):
    def __init__(
        self,
        action_tokenizer: PreTrainedTokenizerBase,
        base_tokenizer: ActionTokenizer,
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        #    ii = i
        #else:
        #    ii = random.randint(0, len(self.index_to_data)-1)
        #ii = i

        while flag_dist:
            f_curr, curr_time, max_goal_dist = self.index_to_data[i]
            #f_curr_x, curr_time_x, _ = self.index_to_data[ii]

            f_goal, goal_time, goal_is_negative = self._sample_goal(f_curr, curr_time, max_goal_dist)

            # random delay
            lt = random.randint(0, 9)
            #lt = 0
            #obs_lt = self._load_image(f_curr, curr_time - lt)
            obs_lt = self._load_image(f_curr, curr_time - lt)
            if obs_lt == None:
                lt = random.randint(0, self.context_size)
                #obs_lt = self._load_image(f_curr, curr_time - lt)
                obs_lt = self._load_image(f_curr, curr_time - lt)
                #print("re-sampling (still random)")
            if obs_lt == None:
                lt = 0
                #obs_lt = self._load_image(f_curr, curr_time - lt)
                obs_lt = self._load_image(f_curr, curr_time - lt)
                #print("re-sampling (lt = 0)")
            ...
            if random.random() > 0.2:
                ...
                if obs_lt == None:
                    lt = 0
                    ii = random.randint(0, len(self.index_to_data)-1)
                    f_curr_x, curr_time_x, max_goal_dist_x = self.index_to_data[ii]
                    f_goal, goal_time, goal_is_negative = self._sample_goal(f_curr_x, curr_time_x, max_goal_dist_x)
                    obs_lt = self._load_image(f_curr_x, curr_time_x)
                    #print("B", obs_lt)
            """
                if obs_lt == None:
                    lt = random.randint(0, self.context_size)
            ...
            obs_images_list = torch.split(obs_image, 3, dim=0)
            #cur_image_large = TF.resize(obs_images_list[-1], (224, 224))
            cur_image_large = TF.resize(obs_lt, (224, 224))

            # Load goal image
            if goal_is_negative:
                goal_image = self._load_image(f_goal, goal_time)
            else:
                goal_image = self._load_image(f_goal, goal_time - lt)
            ...
            # Compute actions
            actions, _, _ = self._compute_actions(curr_traj_data, curr_time, goal_time)
            actions_lt, _, _ = self._compute_actions(curr_traj_data, max(curr_time-8, 0), goal_time) 
            _, goal_pos, goal_yaw = self._compute_actions(curr_traj_data, curr_time-lt, goal_time-lt)

            dist = np.sqrt((actions[7,0]-actions_lt[7,0])**2 + (actions[7,1]-actions_lt[7,1])**2)
            #print(dist)

            if rand_dist > 0.5 and dist > 4.0:
                flag_dist = False
            elif not rand_dist > 0.5:
                flag_dist = False
            else:
                i = random.randint(0, len(self.index_to_data)-1)
            #flag_dist = False
        ...
        return dict(
            ...
            img_PIL=to_pil_image(cur_image_large),
            gimg_PIL=to_pil_image(goal_image),
            p_image = TF.resize(cur_image_large, (96, 96)),
            c_image = TF.resize(torch.split(obs_image, 3, dim=0)[-1], (96, 96)),
            cur_image = obs_image, 
            goal_image_8=goal_image, 
            temp_dist=distance,
            lan_prompt="No language instruction"       
