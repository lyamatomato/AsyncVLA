...
class Frodobots_Dataset_rand(LeRobotDataset):
    def __init__(
        self,
        action_tokenizer: PreTrainedTokenizerBase,
        base_tokenizer: ActionTokenizer,
    ...
    def _image_transforms_rand_crop(self, img: torch.Tensor, flip: bool, top, height, left, width) -> torch.Tensor:
        """Random crop with constrained random offsets (keeps runtime deterministic-ish)."""
        if self.image_transforms is not None:
            img = self.image_transforms(img)
        """
    ...
    def _image_rand_crop_224(self, img: torch.Tensor, flip: bool, top, height, left, width) -> torch.Tensor:
        """Produce a 224x224 random crop variant for visualization/consistency."""
        """
        top = np.random.randint(0, 20)
        height = 144 - np.random.randint(0, 20) - top
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # random delay 
        lt = random.randint(0, min(idx, 9))
        #lt = 0
        episode_length_remaining = self.episode_data_index["to"][ep_id] - idx
        goal_dist = np.random.randint(-lt, min(self.goal_horizon-lt, episode_length_remaining))
        goal_dist2 = min(8, episode_length_remaining)
        goal_dist_gps = np.random.randint(-lt, min(self.goal_horizon2-lt, episode_length_remaining))

        # prepare delta timestamps (add goals)
        delta_timestamps = self.delta_timestamps or {}
        delta_timestamps = {k: list(v) if v is not None else None for k, v in delta_timestamps.items()}
        # add goal offsets to every non-None entry
        extra = [goal_dist * self.dt * self.action_spacing, goal_dist2 * self.dt * self.action_spacing, goal_dist_gps * self.dt * self.action_spacing, -lt * self.dt * self.action_spacing]
        delta_timestamps = {
            k: (v + extra if v is not None else None) for k, v in delta_timestamps.items()
        }
        # control horizon timestamps
        ...
        delta_timestamps["observation.filtered_position"] = delta_timestamps["observation.filtered_heading"] = delta_timestamps["observation.relative_position"] = [
            i * self.dt * self.action_spacing for i in range(control_horizon)
        ] + [goal_dist * self.dt * self.action_spacing] + [goal_dist_gps * self.dt * self.action_spacing] + [-lt * self.dt * self.action_spacing]
        ...
        top_crop = np.random.randint(0, 20)
        height_crop = 144 - np.random.randint(0, 20) - top_crop
        left_crop = np.random.randint(0, 25)
        width_crop = 256 - 2 * left_crop        
        
        image_obs_crop = self._image_transforms_rand_crop(image_obs_raw_s, flip_tf, top_crop, height_crop, left_crop, width_crop)
        image_obs = self._image_transforms(image_obs_raw, flip_tf)
        ...
        image_goal = self._image_transforms_rand_crop(
            ...
            flip_tf, top_crop, height_crop, left_crop, width_crop
        )
        ...
        #image_current, image_raw = self._image_transforms_depth(image_current_shadow, flip_tf)
        image_obs_topil = self._image_rand_crop_224(image_current_shadow, flip_tf, top_crop, height_crop, left_crop, width_crop)
        ...
        unnorm_position = item["observation.filtered_position"][:-2]
        unnorm_position_lt = item["observation.filtered_position"][-1]
        current_heading_lt = item["observation.filtered_heading"][-1]
        current_heading = item["observation.filtered_heading"][0]
        goal_heading = item["observation.filtered_heading"][-2]
        ...
        # compute local goal pose and relative transform
        if flip_tf:
            goal_pos_relative = to_local_coords(item["observation.filtered_position"][-2, None], unnorm_position_lt, current_heading_lt)[0]
            relative_mat = to_local_coords_yaw(item["observation.filtered_position"][-2, None], unnorm_position_lt, current_heading_lt, goal_heading)
        ...
        action_IL = torch.tensor(action_IL)
        #heading = item["observation.filtered_heading"][:-2]
        #direction = torch.stack([torch.cos(heading), torch.sin(heading)], dim=-1)
        ...
        #future_positions_unfiltered = to_local_coords(item["observation.relative_position"][:-2], unnorm_position[0], current_heading)
        which_dataset = 0
        ...
        return dict(
            ...
            img_PIL=pil_image_obs,
            gimg_PIL=pil_image_goal,
            p_image=TF.resize(image_obs_topil, (96, 96)),    
            c_image=TF.resize(torch.split(image_flattened_crop, 3, dim=0)[-1], (96, 96)),
            cur_image=image_flattened,
            goal_image_8=image_goal2,
            temp_dist=goal_dist_gps,
            lan_prompt="No language instruction",
