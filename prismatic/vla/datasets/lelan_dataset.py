...
class LeLaN_Dataset_rand(Dataset):
    def __init__(
        self,
        action_tokenizer: PreTrainedTokenizerBase,
        base_tokenizer: ActionTokenizer,
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        while flag_data == 0:
            ...
            #lt = random.randint(0, min(iv, 6))
            lt = random.randint(0, min(iv, 3))

            #lt = 0
            image_fullsize_PIL = self._load_image_front_PIL(self.image_path[iv-lt])                            
            image_fullsize = self._load_image_front(self.image_path[iv])                        
            context_image = [image_fullsize]        
            for ih in range(self.context_size):
                if iv-ih > 0:                   
            ...
            try:
                gimage_fullsize_PIL = self._load_image_front_PIL(self.image_path[iv + goal_id - lt])              
            except:
                goal_id = 0
                gimage_fullsize_PIL = self._load_image_front_PIL(self.image_path[iv + goal_id - lt])
                
            cimage_fullsize_PIL = self._load_image_front_PIL(self.image_path[iv])
