...
class BDD_Dataset_rand(Dataset):
    def __init__(
        self,
        action_tokenizer: PreTrainedTokenizerBase,
        base_tokenizer: ActionTokenizer,
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        while flag_data == 0:
            # random delay 
            #lt = random.randint(0, min(iv, 9))            
            lt = random.randint(0, min(iv,1))  
            #lt = 0
            image_fullsize_lt = self._load_image_front(self.image_path[iv-lt][0])
            PIL_current = to_pil_image(image_fullsize_lt).resize((224, 224))
            ...
            #goal_id = random.randint(0,5)
            goal_id = 1
            cur_pos = self.gps_list[iv - lt]    
            mbra_traj = self.mbra_traj_list[iv]                        
            try:
                goal_image_full = self._load_image_front(self.image_path[iv + goal_id - lt][0])
                goal_image_full_8 = self._load_image_front(self.image_path[iv + 1][0])     
                PIL_goal = to_pil_image(goal_image_full).resize((224, 224))
                
                curr_path = self.image_path[iv - lt][0]   
                goal_path = self.image_path[iv + 1 - lt][0]           
                goal_pos = self.gps_list[iv + goal_id -lt]
            except:
                goal_id = 0
                goal_image_full = self._load_image_front(self.image_path[iv + goal_id - lt][0])   
                goal_image_full_8 = self._load_image_front(self.image_path[iv + goal_id][0])   
                PIL_goal = to_pil_image(goal_image_full).resize((224, 224))
                goal_pos = self.gps_list[iv + goal_id - lt]     
                curr_path = self.image_path[iv - lt][0]   
                goal_path = self.image_path[iv + goal_id -lt][0]   
        ...
        return dict(
            ...
            img_PIL=PIL_current_crop_r,
            gimg_PIL=PIL_goal_crop_r,
            p_image=TF.resize(to_tensor(PIL_current_crop_r), (96, 96)),
            c_image=TF.resize(torch.split(image_crop_r, 3, dim=0)[-1], (96, 96)),              
            cur_image = image_obs_r, 
            goal_image_8=goal_image_full_8_r, 
            temp_dist=goad_id_virtual,
            lan_prompt="No language instruction"            
